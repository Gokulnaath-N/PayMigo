...
const Login = () => {
  const { user, loading: authLoading, setLoading, setUser } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // User is already signed in via Google — no need to apply custom token.
      // Just store the backend JWT and worker profile.
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        // Sign out any existing Firebase session before applying custom token
        ...
        const w = data.worker;
        if (!w.uid) w.uid = w.firebaseUid || String(w.id);
        if (!w.displayName) w.displayName = w.name || w.email;
        setUser(w);
        navigate('/onboard', { replace: true });
