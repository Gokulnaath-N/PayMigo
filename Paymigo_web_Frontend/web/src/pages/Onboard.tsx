...
const Onboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchMLPremium = async () => {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/premium/calculate`, {
          method: 'POST',
          headers: {
  ...
  const saveUserData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/workers/onboard`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      // Normalize uid so ProtectedRoute and Firestore queries work correctly
      if (!data.uid) data.uid = data.firebaseUid || String(data.id);
      if (!data.displayName) data.displayName = data.name || data.email;

      localStorage.setItem('paymigo_user', JSON.stringify(data));

      // ── Critical fix: push the updated worker into AuthContext so
      // ProtectedRoute sees a valid user and navigate('/dashboard') works.
      setUser(data);
        ...
        return (
          ...
          >
            ...
            <div className="glass-card p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="text-xs font-bold uppercase tracking-widest text-text-secondary">Payment Method</div>
                <RazorpayPayment
              ...
              <div className="flex items-center gap-3 p-4 bg-success/10 border border-success/20 rounded-xl">
                ...
                <p className="text-[10px] text-success/80">Secure payment gateway. {isMonsoon ? "Monsoon Stay-at-Home benefit active." : "Protection starts instantly after payment."}</p>
              </div>
            </div>
          </motion.div>
        );
