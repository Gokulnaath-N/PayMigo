...
const Dashboard = () => {
    ...
    const unsubWorker = onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          // Only redirect to onboard if we have a real Firebase UID but no Firestore doc
  ...
  useEffect(() => {
    ...
    const fetchTriggers = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/triggers?mode=${weatherMode}`);
        ...
        if (data.status === 'PAYOUT_TRIGGERED' && triggerStatusRef.current !== 'PAYOUT_TRIGGERED') {
          try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/ai/trigger-payout`, {
              workerId: user.uid,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      {/* Curfew Alert Banner */}
      <CurfewBanner userZone={workerData?.zoneId || 'chennai-central'} />
          ...
          <div className="flex items-center gap-4">
            ...
            {activeTab === 'dashboard' && (
              ...
              >
                ...
                <option value="fraud">Mock: Fraudulent Activity</option>
              </select>
            )}
