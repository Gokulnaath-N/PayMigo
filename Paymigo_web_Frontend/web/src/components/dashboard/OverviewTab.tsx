...
const OverviewTab = ({
  ...
  <div className="space-y-8">
    {/* Trigger-Based Payment Prompt */}
    {triggerStatus === 'PAYOUT_TRIGGERED' && !paymentSuccess && (
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="glass-card p-6 border-2 border-accent/50 bg-accent/5"
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-grow">
            <h3 className="text-lg font-bold mb-2">Renew Your Protection Now</h3>
            <p className="text-sm text-text-secondary mb-4">
              Trigger activated! Secure your coverage for next week while conditions are active.
            </p>
            {!showPayment ? (
              <button
                onClick={() => setShowPayment(true)}
                className="px-6 py-3 bg-accent text-white rounded-xl font-bold hover:scale-105 transition-all shadow-lg shadow-accent/20"
              >
                Renew Premium - ₹{workerData?.weeklyPremium || 69}
              </button>
            ) : (
              <div className="mt-4">
                <RazorpayPayment
                  ...
                  planName={workerData?.plan || 'Pro'}
                  workerId={user?.uid || user?.id || ''}
                  onSuccess={(data) => {
                    console.log('Renewal payment successful:', data);
    ...
    {paymentSuccess && (
      <motion.div
        ...
        className="glass-card p-6 border-2 border-success/50 bg-success/5"
      >
        <div className="flex items-center gap-3">
          <CheckCircle2 className="w-6 h-6 text-success" />
          <div>
            <h3 className="font-bold text-success">Premium Renewed Successfully!</h3>
            <p className="text-sm text-text-secondary">Your protection is active for the next 7 days.</p>
          </div>
        </div>
      </motion.div>
    )}
      ...
      <div className="space-y-8">
        ...
        <div className="glass-card p-6">
          ...
          <button 
            onClick={async () => {
              if (!user) return;
              const amount = walletData?.availableBalance || 0;
              if (amount <= 0) {
              ...
              try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/wallet/withdraw`, {
                  method: 'POST',
                  headers: {
                ...
                if (data.success) {
                  alert(`Withdrawal successful! ₹${amount} transferred to your UPI.`);
            ...
            className="w-full py-4 bg-accent text-background rounded-xl font-bold flex items-center justify-center gap-2 hover:glow-accent transition-all"
          >
    ...
    {/* ── Step 5: Risk Forecasting Summary Widget ── */}
    <RiskSummaryWidget />
  </div>
  );
};
